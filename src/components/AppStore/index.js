...
const socialList = appsList.filter(eachItem => eachItem.category === 'SOCIAL')

class AppStore extends Component {
  state = {activeApps: socialList}

  onChangeOfCategory = tabId => {
    const filteredApps = appsList.filter(
      eachItem => eachItem.category === tabId,
    )
    this.setState({activeApps: filteredApps})
  }

  onSearch = event => {
    const searchInput = event.target.value
    const {activeApps} = this.state
    const searchResults = activeApps.filter(eachItem =>
      eachItem.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )
    console.log(searchResults)
    this.setState({
      activeApps: searchResults,
    })
  }

  render() {
    const {activeApps} = this.state
    const {searchResults} = this.onSearch
    return (
        ...
        <ul className="ulist">
          {tabsList.map(eachItem => (
            <TabItem
              tabsList={eachItem}
              key={eachItem.tabId}
              onChangeOfCategory={this.onChangeOfCategory}
              isActive={eachItem.tabId === activeApps[0].category}
            />
          ))}
        </ul>

        <ul className="ulist2">
          {activeApps.map(eachItem => (
            <AppItem appsList={eachItem} key={eachItem.appId} />
          ))}
        </ul>
      </div>
    )
